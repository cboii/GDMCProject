...
def build_wooden_roof(editor: Editor, tile_array: npt.NDArray[any], wood_type: str):
    ...
    for i in range(tile_array.shape[0],-1,-1):
        for j in range(tile_array.shape[2],-1,-1):
            name, rotation = tile_array[i][-1][j].selected_module
            if name.endswith("Corner") and rotation == 1:
                se_corner_tile = tile_array[i][-1][j]
                break

    nw_corner_coordinate = ivec3(nw_corner_tile.pos.x, nw_corner_tile.pos.y + TILE_SIZE.y, nw_corner_tile.pos.z) 
    se_corner_coordinate = ivec3(se_corner_tile.pos.x + TILE_SIZE.x, se_corner_tile.pos.y + TILE_SIZE.y, se_corner_tile.pos.z + TILE_SIZE.z)
...
def generic_wooden_roof(editor: Editor, length: int, width: int, wood_type: str):
    ...
    for i in range(width//2 + 1):
        placeCuboid(editor, (-1+i, i, -1), (-1+i, i, length+1), east_block)
        placeCuboid(editor, (width-i, i, -1), (width-i, i, length+1), west_block)
        
        if i < width//2:
            if i % 2 == 1:
                placeCuboid(editor, (i, 1, 0), (i,1+i,0), Block(planks_name))
                placeCuboid(editor, (width-1-i, 1, 0), (width-1-i,1+i,0), Block(planks_name))
                placeCuboid(editor, (i, 1, length), (i,1+i,length), Block(planks_name))
                placeCuboid(editor, (width-1-i, 1, length), (width-1-i,1+i,length), Block(planks_name))
            else:
                placeCuboid(editor, (i, 1, 0), (i,1+i,0), Block(log_name))
                placeCuboid(editor, (width-1-i, 1, 0), (width-1-i,1+i,0), Block(log_name))
                placeCuboid(editor, (i, 1, length), (i,1+i,length), Block(log_name))
                placeCuboid(editor, (width-1-i, 1, length), (width-1-i,1+i,length), Block(log_name))
            
            editor.placeBlock((i, i, -1), east_flipped_block)
            editor.placeBlock((width-1-i, i, -1), west_flipped_block)
            editor.placeBlock((i, i, length+1), east_flipped_block)
            editor.placeBlock((width-1-i, i, length+1), west_flipped_block)
        elif width % 2 == 1:
            placeCuboid(editor, (width//2, i, -1), (width//2, i, length+1), Block(planks_name))
            placeCuboid(editor, (width//2, i+1, -1), (width//2, i+1, length+1), Block(slab_name))
            placeCuboid(editor, (width//2, 1, 0), (width//2, i, 0), Block(log_name))
            placeCuboid(editor, (width//2, 1, length), (width//2, i, length), Block(log_name))
