...
def get_entrance_direction(area: Rect, road_map) -> int:
    ...
    if nearest_rb.y <= area.offset.y+1:
        if area.offset.y+1 - nearest_rb.y < area.offset.x+1 - nearest_rb.x:
            return 3
        elif area.offset.y+1 - nearest_rb.y < nearest_rb.x - area.last.x-1:
            return 1
        else:
            return 0
    elif nearest_rb.y >= area.last.y-1:
        if nearest_rb.y - area.last.y-1 < area.offset.x+1 - nearest_rb.x:
            return 3
        elif nearest_rb.y - area.last.y-1 < nearest_rb.x - area.last.x-1:
            return 1
        else:
            return 2
    else:
        if area.offset.x+1 >= nearest_rb.x:
            return 3
        elif area.last.x-1 <= nearest_rb.x:
            return 1
